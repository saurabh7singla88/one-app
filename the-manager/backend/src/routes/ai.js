...
import {
  loadAISettings, callLLM, callOllama, callOpenAI, callGemini, callBedrock,
  LLM_TIMEOUT, ENCRYPTED_SETTING_KEYS,
} from '../lib/llm.js';
...
async function analyseWithLLM(items, settings) {
  ...
  if (toAnalyse.length === 0) {
    logger.info('AI urgency analysis skipped — no items with descriptions', { provider, totalItems: items.length });
    return { map: {}, provider };
  }
